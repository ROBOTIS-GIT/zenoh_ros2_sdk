...
class ZenohSession:
    ...
    def __init__(self, router_ip: str = "127.0.0.1", router_port: int = 7447):
        ...
        # Apply user overrides first (mode, scouting, etc.)
        override = os.environ.get("ZENOH_CONFIG_OVERRIDE", "").strip()
        override_has_endpoints = False
        if override:
            override_has_endpoints = "connect/endpoints" in override
            _apply_zenoh_config_override(self.conf, override)

        # Set default connect/endpoints AFTER overrides to prevent
        # mode="peer" from clearing it. Skip if user explicitly set endpoints.
        if not override_has_endpoints:
            self.conf.insert_json5(
                "connect/endpoints", f'["tcp/{router_ip}:{router_port}"]'
            )

        # Enable shared memory transport if ZENOH_SHM_ENABLED=true
        # and not already set via override.
        shm_enabled = os.environ.get("ZENOH_SHM_ENABLED", "").lower() == "true"
        override_has_shm = "transport/shared_memory" in override if override else False
